...
// Si la machine est big-endian, on swappe les octets
static inline u32 bswap32(u32 x) {
    ...
    // Little-endian machine : rien à faire
    static inline u32 le32_to_cpu(u32 x) { return x; }
    static inline u32 cpu_to_le32(u32 x) { return x; }
#else
    // Big-endian machine : swap
    static inline u32 le32_to_cpu(u32 x) { return bswap32(x); }
    static inline u32 cpu_to_le32(u32 x) { return bswap32(x); }
#endif

struct serpent_ctx {
	unsigned char iv[SERPENT_BLOCK_SIZE];
	u32 expkey[SERPENT_EXPKEY_WORDS];
};

static void setkey(void *ctx, const u8 *key, unsigned int keylen)
{
