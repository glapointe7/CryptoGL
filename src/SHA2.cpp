...
const BytesVector SHA32Bits::process(const BytesVector &data, const uint8_t truncate_to)
{
...
const BytesVector SHA64Bits::process(const BytesVector &data, const uint8_t truncate_to)
{
   BytesVector bytes = appendPadding(data);
   appendLength<BigEndian64>(bytes, data.size() << 3);
...
const BytesVector SHA224::encode(const BytesVector &data)
{
   return process(data, 7);
}

const BytesVector SHA256::encode(const BytesVector &data)
{
   return process(data, 8);
}

const BytesVector SHA384::encode(const BytesVector &data)
{
   return process(data, 12);
}

const BytesVector SHA512::encode(const BytesVector &data)
{
   return process(data, 16);
}

const BytesVector SHA512_224::encode(const BytesVector &data)
{
...
const BytesVector SHA512_256::encode(const BytesVector &data)
{
