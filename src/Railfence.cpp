...
void RedefenceZigzag::addEncodedText(Vector<ClassicalType> &rows, const ClassicalType &data,
...
ClassicalType
RedefenceZigzag::swapRowsWithPermutationKey(const Vector<ClassicalType> &rows, const uint32_t length) const
{
    ClassicalType crypted(length);
...
ClassicalType RedefenceZigzag::encode(const ClassicalType &clear_text)
{
    const uint32_t clear_len = clear_text.length();
    Vector<ClassicalType> rows;
...
RedefenceZigzag::getFirstDecoding(const ClassicalType &cipher_text, int32_t &last) const
{
    ...
    Vector<std::list<int8_t> > decoding;
...
ClassicalType RedefenceZigzag::decode(const ClassicalType &cipher_text)
{
    ...
    Vector<std::list<int8_t> > first_decoded = getFirstDecoding(cipher_text, last_char_level);
    ...
    while (!first_decoded[last_char_level].empty())
    {
        ...
        {
            decrypted.push_back(first_decoded[k].front());
