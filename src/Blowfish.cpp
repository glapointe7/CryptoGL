        ...
        const int8_t lower_round, const int8_t round_max)
{
   ...
   int8_t final_rounds = upper_round + 1;
   if (lower_round < 0)
   {
      is_increasing = -1;
      final_rounds = 1;
   ...
   for (int8_t round = lower_round; round <= upper_round; ++round)
   {
      ...
      L ^= subkeys[positive_round];
      R ^= F(L, subkeys[positive_round]);
   ...
   R ^= subkeys[final_rounds];
   L ^= subkeys[final_rounds + is_increasing];
}
...
uint64_t Blowfish::F(const uint64_t &data, const uint64_t &) const
{
   ...
   return (((sboxes[0][V[0]] + sboxes[1][V[1]]) & 0xFFFFFFFF) ^ (sboxes[2][V[2]])) + (sboxes[3][V[3]] & 0xFFFFFFFF);
}

const Blowfish::BytesContainer
Blowfish::getOutputBlock(const BytesContainer &data, const UInt64Container &subkeys, const int8_t lower_round)
{
   uint64_t value = 0;
