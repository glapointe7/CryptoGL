/*
 * Source : http://www.gsma.com/technicalprojects/wp-content/uploads/2012/04/snow3gspec.pdf
 * **** Must use the state machine design pattern.
 * We change of state each time it's clocked.
 */
#pragma once

#include "../common/SynchronousStreamCipher.hpp"
#include <array>

namespace CryptoGL
{
    class Snow3G : public SynchronousStreamCipher<UInt32Vector, BigEndian32, 1, BytesVector>
    {
    public:
        Snow3G(const BytesVector &key, const BytesVector &IV, const uint64_t &output_size)
            : output_size(output_size)
        {
            setIV(IV);
            setKey(key);
            keySetup();
        }

        UInt32Vector generateKeystream() override;

        void setIV(const BytesVector &IV) override;

    private:
        const uint64_t output_size;

        /*16 LFSR stages of 32 bits each. */
        UInt32Vector state;

        /* The registers of the FSM. */
        uint32_t R1, R2, R3;

        void keySetup() override;

        void nextState(const uint32_t v);
        uint32_t getv() const;
        void initializationMode(const uint32_t F);
        void keystreamMode();

        static constexpr uint8_t mulx(const uint8_t V, const uint8_t c);
        static constexpr uint8_t mulxPow(const uint8_t V, const uint8_t i, const uint8_t c);
        static uint32_t mulAlpha(const uint8_t c);
        static uint32_t divAlpha(const uint8_t c);

        static uint32_t S(const uint32_t w, const std::array<uint8_t, 256> &sbox, const uint8_t c);
        static uint32_t S1(const uint32_t w);
        static uint32_t S2(const uint32_t w);

        uint32_t finiteStateMachine();

        /* The first sbox is the Rijndael sbox. */
        static constexpr std::array<uint8_t, 256> SR = {{
            
        }};

        static constexpr std::array<uint8_t, 256> SQ = {{
            
        }};
    };
}