...
namespace CryptoGL
{
    ...
    public:
        ...
        DES(const BytesVector &key, const OperationModes mode, const BytesVector &IV)
            : Feistel(mode, 16, IV)
        {
            setKey(key);
        }
        
        explicit DES(const BytesVector &key)
            : DES(key, OperationModes::ECB, {}) { }
    ...
    private:
        ...
        {
            uint8_t i = TableSize - 1;
            uint64_t output = 0;

            for (uint8_t k = 0; k < TableSize; ++k, --i)
            {
        ...
        /* Expansion table. */
        static constexpr std::array<uint8_t, 48> E = {{
            32, 1, 2, 3, 4, 5,
            4, 5, 6, 7, 8, 9,
            8, 9, 10, 11, 12, 13,
            12, 13, 14, 15, 16, 17,
        ...
        /* Substitution tables. */
        static constexpr std::array<std::array<std::array<uint8_t, 16>, 4>, 8> S = {{
            {{
