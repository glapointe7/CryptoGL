...
void RC5::encodeFeistelRounds(uint64_t &L, uint64_t &R, const uint8_t) const
{
   L += subkeys[0];
   R += subkeys[1];
   ...
   {
      const uint8_t j = i << 1;
      L = rotateLeft((L ^ R) & 0xFFFFFFFF, R & 31, 32) + subkeys[j];
      R = rotateLeft((R ^ L) & 0xFFFFFFFF, L & 31, 32) + subkeys[j + 1];
   }
}

void RC5::decodeFeistelRounds(uint64_t &L, uint64_t &R, const uint8_t) const
{
   for (uint8_t i = rounds; i > 0; --i)
   {
      const uint8_t j = i << 1;
      R = rotateRight((R - subkeys[j + 1]) & 0xFFFFFFFF, L & 31, 32) ^ L;
      L = rotateRight((L - subkeys[j]) & 0xFFFFFFFF, R & 31, 32) ^ R;
   }
   R -= subkeys[1];
   L -= subkeys[0];
}

const RC5::BytesContainer RC5::getOutputBlock(const BytesContainer &block, const bool to_encode)
{
   LittleEndian32 LE;
   ...
   LE.toBytes(L);
   BytesContainer tmp = LE.getBytes();
...
const RC5::BytesContainer RC5::encode(const BytesContainer &clear_text)
{
   return processEncoding(clear_text);
}

const RC5::BytesContainer RC5::decode(const BytesContainer &cipher_text)
{
