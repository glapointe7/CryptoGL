...
Fleissner::Fleissner(const CoordinatesVector &key, const uint32_t grid_dim, const bool clockwise)
    : grid_dim(grid_dim)
{
    CoordinatesVector coords(key);
    const uint32_t key_size = key.size();
    const uint32_t mask_size_approuved = key_size * 4;

    if (mask_size_approuved != (grid_dim * grid_dim))
    {
        throw BadKeyLength("Fleissner (constructor): Your key has to be square following the dimension you have provided.", mask_size_approuved);
    }
    ...
    CoordinatesSet rotation;

    if (clockwise)
    {
        for (const auto &coord : key)
        {
            rotationExists(rotation, coord.y, grid_dim - 1 - coord.x);
        }
    }
    else
    {
        for (const auto &coord : key)
        {
            rotationExists(rotation, grid_dim - 1 - coord.y, coord.x);
        }
    }
    ...
    for (const auto &coord : key)
    {
        rotationExists(rotation, grid_dim - 1 - coord.x, grid_dim - 1 - coord.y);
    }
    coords.insert(coords.end(), rotation.begin(), rotation.end());
    rotation.clear();
...
void Fleissner::rotationExists(CoordinatesSet &rotation, const uint32_t x, const uint32_t y)
{
    const auto is_unique = rotation.insert(Coordinates(x, y));
...
ClassicalType Fleissner::encode(const ClassicalType &clear_text)
{
    ...
    for (uint32_t i = 0; i < max_grid; ++i)
    {
        ...
        {
            grid[key[j].x][key[j].y] = full_text[k];
        }
...
ClassicalType Fleissner::decode(const ClassicalType &cipher_text)
{
    ...
    for (uint32_t i = 0; i < max_grid; ++i)
    {
        ...
        {
            decrypted.push_back(grid[key[c].x][key[c].y]);
        }
    }
