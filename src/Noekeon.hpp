...
class Noekeon : public BlockCipher<uint32_t, std::vector<uint32_t> >
{
public:
   explicit Noekeon(const BytesContainer &key) : BlockCipher<uint32_t, std::vector<uint32_t> >(OperationModes::ECB, 16) { setKey(key); }
   Noekeon(const BytesContainer &key, const OperationModes mode) : BlockCipher<uint32_t, std::vector<uint32_t> >(mode, 16) { setKey(key); }
