...
void Graph::addVertex(const uint32_t ID)
{
    const Vertex V(ID);
    Vertices.push_back(V);
}

void Graph::addEdge(const uint32_t ID_left, const uint32_t ID_right)
{
    const Vertex U(ID_left);
    const Vertex V(ID_right);
    const Edge E(U, V);
    Edges.push_back(E);
}
...
void Graph::load(const String &filename)
{
    ...
    while (std::getline(InputFile, Line)) 
    {
        ...
        uint32_t vertex_ID, edge_ID;
        InputString >> vertex_ID;
        addVertex(vertex_ID);
        while(InputString >> edge_ID)
        {
            addEdge(vertex_ID, edge_ID);
        }
    }
}

Edge Graph::pickRandomEdge() const
{
    const uint32_t edges_length = Edges.size();
    const Edge E = Edges[rand() % edges_length];
    
    return E;
}

void Graph::randomContraction()
{
    const Edge RandomEdge = pickRandomEdge();

    Vertex Left = Vertices[RandomEdge.getLeft().getID() - 1];
    Vertex Right = Vertices[RandomEdge.getRight().getID() - 1];
    
    Vector<Edge> NewEdgesVector;
    for (const Edge &E : Edges)
    {
        Edge NewEdge = E;
        if (NewEdge.getLeft().getID() == Right.getID()) 
        {
            // re assign edge to point to v1
            NewEdge.getLeft().setID(Left.getID());
        }
        if (NewEdge.getRight().getID() == Right.getID()) 
        {
            NewEdge.getRight().setID(Left.getID());
        }
        // only add it to the new list if it's not a self-loop
        if (!NewEdge.isLoop()) 
        {
            NewEdgesVector.push_back(NewEdge);
        } 
    }
    Edges = NewEdgesVector;
    //Vertices.erase();
}

// Compute the MinCut by making repeated attempts at the random contraction algorithm.
uint32_t Graph::getMinimumCut() const
{
    const uint32_t trials = Vertices.size() * Vertices.size() * log(Vertices.size());
    uint32_t best_cut_result = Edges.size();
    ...
    for (uint32_t i = 0; i < trials; ++i) 
    {
        ...
        uint32_t edges_length = G.getEdges().size();
        if (edges_length < best_cut_result) 
        {
            best_cut_result = edges_length;
        }
    }
    
    return best_cut_result;
}
