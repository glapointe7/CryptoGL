...
{   
   EXPECT_EQ("EA6552BA793546C261E4B3E90433F5A2", Digest::hexDigest(R->encode(std::vector<uint8_t>(16, 0))));
}

TEST_F(NoekeonTest, decodeZero)
{
   EXPECT_EQ("00000000000000000000000000000000", Digest::hexDigest(R->decode(Digest::getBytesFromHexDigest("EA6552BA793546C261E4B3E90433F5A2"))));
}
...
TEST_F(NoekeonTest, encodeNormal)
{
   ...
   EXPECT_EQ("EA024714AD5C4D84EA024714AD5C4D84", Digest::hexDigest(C->encode(clear_text)));
}

TEST_F(NoekeonTest, decodeNormal)
{
   EXPECT_EQ("4765F3DA10CD3D0473867742B5E5CC3C", Digest::hexDigest(C->decode(Digest::getBytesFromHexDigest("EA024714AD5C4D84EA024714AD5C4D84"))));
}
