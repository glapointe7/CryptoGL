...
TEST_F(BlakeTest, encode512ShortText)
{
   ...
   EXPECT_EQ(hash, Digest::hexDigest(S->encode(Digest::getBytesFromHexDigest(clear_text))));
}
...
TEST_F(BlakeTest, encode384ShortText)
{
   ...
   EXPECT_EQ(hash, Digest::hexDigest(T->encode(Digest::getBytesFromHexDigest(clear_text))));
}
...
TEST_F(BlakeTest, encode256ShortText)
{
   ...
   EXPECT_EQ(hash, Digest::hexDigest(U->encode(Digest::getBytesFromHexDigest(clear_text))));
}
...
TEST_F(BlakeTest, encode224ShortText)
{
   ...
   EXPECT_EQ(hash, Digest::hexDigest(V->encode(Digest::getBytesFromHexDigest(clear_text))));
}
