...
namespace UnitTests
{
    ...
    {   
       compare(StringTest("21A5DBEE154B8F6D"), R->encode(BytesVector(8, 0)).toHexString());
    }

    TEST(RC5DecodeZeroValues, RC5Test)
    {
       compare(StringTest("0000000000000000"), R->decode(StringTest("21A5DBEE154B8F6D").hexToBytes()).toHexString());
    }

    TEST(RC5EncodeNormalCase, RC5Test)
    {
       const BytesVector clear_text = {0x21, 0xA5, 0xDB, 0xEE, 0x15, 0x4B, 0x8F, 0x6D};
       compare(StringTest("F7C013AC5B2B8952"), C->encode(clear_text).toHexString());
    }

    TEST(RC5DecodeNormalCase, RC5Test)
    {
       compare(StringTest("21A5DBEE154B8F6D"), C->decode(StringTest("F7C013AC5B2B8952").hexToBytes()).toHexString());
    }
}
